...
public class MavenOverseer{
	...
	private static Logger logger = Logger.getLogger(MavenOverseer.class);
	...
	public String listUpdateableDependencies() {
		...
		listDeps.append("\"The following dependencies can be updated to the versions listed:"+System.lineSeparator());
		...
		for (int i = 0; i < dependencyList.size(); i++) {
			...
			listDeps.append(curr.groupID + ":" + curr.artifactID + ":" + curr.currVersion + ":" + curr.newerVersions+System.lineSeparator());
		}
		listDeps.append("\"");
		if (atLeastOneUpdate) {
	...
	private void findDependencies() {
		ProcessBuilder tempBuilder = new ProcessBuilder(getExecutablePath(), getDefaultArg(), 
				"cd " + projectUpdateLocation + " && mvn clean install");
		
		
		
		ProcessBuilder builder = new ProcessBuilder(getExecutablePath(), getDefaultArg(), 
				"cd " + projectUpdateLocation + " && mvn dependency:list");
		builder.redirectErrorStream(true);
